...
def degree_heuristic(unassigned_list):
    #choose the variable in static order that is involved in the largest number of constraints
    connections_index = 2
    max_constraints = 0
    max_degree_variables = []
    for variable in unassigned_list:
        if max_constraints < len(variable[connections_index]):
            max_constraints = len(variable[connections_index])
            max_degree_variables.append(variable)
    return max_degree_variables[0]

def select_unassigned_variable(unassigned_list):
    #returns first node in ordered list
    return degree_heuristic(unassigned_list)

def remove_assigned_variable(selected_variable, unassigned_list, assigned_list):
    print selected_variable
    print assigned_list
    print unassigned_list
    index = unassigned_list.index(selected_variable)
    #pdb.set_trace()
    assigned_list.insert(0,unassigned_list.pop(index))
...
def least_constraining_values(selected_variable, unassigned_list): 
    constraint_values_index = 1
    constraint_values = selected_variable[constraint_values_index]
    constrained_values_sum = {constraint_value: 0 for constraint_value in constraint_values }
    neighbors = neighbor_list_generator(selected_variable, unassigned_list)
     
    for constraint_value in selected_variable[constraint_values_index]:
        for neighbor in neighbors:
            if constraint_value in neighbor[constraint_values_index]:
                constrained_values_sum[constraint_value] = constrained_values_sum[constraint_value] + 1
    pdb.set_trace()
...
def order_domain_values(selected_variable, unassigned_list):
    ...
    values = least_constraining_values(selected_variable, unassigned_list)
    values = values[0]
    return values
...
def make_arc_consistent(selected_variable, unassigned_list,assigned_list):
    pre_changed_variables = []
    checklists = [unassigned_list,assigned_list]
    constraints_index = 1
...
def restore(unassigned_list, assigned_list, pre_changed_variables):
...
def inference(selected_variable, unassigned_list, assigned_list):
    #for backtrack this is not used unless we applies an inference algorithm like forward checking
    forward_checking(selected_variable, unassigned_list, assigned_list)
    ...
    while len(unassigned_list) != 0:
        ...
        for value in order_domain_values(selected_variable, unassigned_list):
            ...
            if is_consistent_value(value, selected_variable, unassigned_list, assigned_list):
                ...
                inference(selected_variable, unassigned_list, assigned_list)
...
color_map(connecticut, ["red", "blue", "green","yellow"], trace=True)
