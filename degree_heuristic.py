...
def find_all_max_degree_variables(max_value, unassigned_list):
    #pdb.set_trace()
    print max_value
    connections_index = 2
    max_degree_variables = []
    for var in unassigned_list:
        if max_value == len(var[connections_index]):
            max_degree_variables.append(var)
    return max_degree_variables
...
def degree_heuristic(unassigned_list):
    ...
    #pdb.set_trace()
    connections_index = 2
    max_degree_by_index = {var[0]:len(var[connections_index]) for var in unassigned_list}
    max_key = max(max_degree_by_index, key=lambda k: max_degree_by_index[k])
    max_degree_variables = find_all_max_degree_variables(max_degree_by_index[max_key], unassigned_list)
    #pdb.set_trace()
    shuffle(max_degree_variables)
    print "max: ",max_degree_variables
...
def inference(selected_variable, unassigned_list):
    ...
    return forward_checking(selected_variable, unassigned_list)
    ...
    while len(unassigned_list) != 0:
        ...
        for value in order_domain_values(selected_variable, unassigned_list):
            ...
            if is_consistent_value(value, selected_variable, unassigned_list, assigned_list):
                ...
                inference_result = inference(selected_variable, unassigned_list)
                break
            #print "inconsistent value ", value
                
            if not inference_result or attempts == values_length:
                print "READY TO BACKTRACK!!!"
...
color_map(europe, ["red", "blue", "green","yellow"], trace=True)
